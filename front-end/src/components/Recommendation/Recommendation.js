...
export default function Recommendation({ name, youtubeLink, score, id, onUpvote = () => 0, onDownvote = () => 0 }) {
  ...
  return (
    <Container>
      <Row id="nameMusic">{name}</Row>
      <ReactPlayer url={youtubeLink} width="100%" height="100%" />
      <Row>
        <GoArrowUp size="24px" data-test-id={`test-upvote-${id}`} onClick={handleUpvote} />
        {score}
        <GoArrowDown size="24px" data-test-id={`test-downvote-${id}`} onClick={handleDownvote} />
      </Row>
    </Container>
  );
}
